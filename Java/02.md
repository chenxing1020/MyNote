...
public class Run{
    public static void main(String[] args){
        AddCountThread[] adt=new AddCountThread[100];
        for (int i=0;i<100;i++) adt[i]=new AddCountThread();
        for (int i=0;i<100;i++) adt[i].start();
    }
}
...
public class Run{
    public static void main(String[] args){
        AddCountThread adt=new AddCountThread();
        Thread t1=new Thread(adt);
        t1.start();
        Thread t2=new Thread(adt);
        t2.start();
        Thread t3=new Thread(adt);
        t3.start();
    }
}
```

&emsp;&emsp;上面的程序运行结果显示，虽然t1、t2、t3三个线程交叉执行，但是每一次的count则是有序的加到30000。  

&emsp;&emsp;这里产生一个疑问：在上面volatile的例子中，换成原子类依然会出现乱序，还需要继续研究一下！
